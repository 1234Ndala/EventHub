      ...
      <nav style={{ backgroundColor: "#1a1a2e", padding: "16px 32px", display: "flex", gap: "24px", alignItems: "center" }}>
        ...
        {["events", "participants", "registrations"].map(p => (
          <button key={p} onClick={() => setPage(p)} style={{
            ...
            borderRadius: "6px", cursor: "pointer", fontSize: "14px", textTransform: "capitalize"
...
function Events() {
  ...
  const load = () => fetch(`${EVENTS_URL}/events`).then(r => r.json()).then(setEvents).catch(() => setMsg("Impossible de charger les événements. Vérifiez que le service tourne."));
...
function Registrations() {
  ...
  const load = () => fetch(`${REGISTRATIONS_URL}/registrations`).then(r => r.json()).then(setRegistrations).catch(() => setMsg("Impossible de charger les inscriptions."));
  ...
  const inscrire = async () => {
    ...
    if (r.ok) { setMsg("Inscription réussie"); setForm({ event_id: "", participant_id: "" }); load(); }
  ...
  const voirStats = async () => {
    const r = await fetch(`${REGISTRATIONS_URL}/registrations/stats/${statsId}`);
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginBottom: "32px" }}>
        <div style={{ background: "white", padding: "24px", borderRadius: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          ...
          {[["event_id","ID Événement"],["participant_id","ID Participant"]].map(([k,l]) => (
            <div key={k} style={{ marginBottom: "12px" }}>
              <label style={{ display: "block", marginBottom: "4px", fontSize: "13px", color: "#555" }}>{l}</label>
              <input value={form[k]} onChange={e => setForm({ ...form, [k]: e.target.value })}
                style={{ width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #ddd", fontSize: "14px", boxSizing: "border-box" }} />
        ...
        <div style={{ background: "white", padding: "24px", borderRadius: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
          ...
          <input value={statsId} onChange={e => setStatsId(e.target.value)} placeholder="ID de l'événement"
            style={{ width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #ddd", fontSize: "14px", boxSizing: "border-box", marginBottom: "12px" }} />
      ...
      <div style={{ background: "white", borderRadius: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1a1a2e", color: "white" }}>
              {["ID","Événement","Participant","Date","Statut","Action"].map(h => (
                <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontSize: "13px" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {registrations.map((r, i) => (
              <tr key={r.id} style={{ background: i % 2 === 0 ? "#fafafa" : "white" }}>
                ...
                <td style={{ padding: "12px 16px", fontSize: "13px" }}>#{r.event_id}</td>
                <td style={{ padding: "12px 16px", fontSize: "13px" }}>#{r.participant_id}</td>
